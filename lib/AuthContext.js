...
const API_BASE = "https://unsafe-leticia-gameless.ngrok-free.dev";
...
async function apiPost(path, body) {
  ...
  try {
    ...
    console.log("[apiPost] JSON 파싱 실패, raw body:", text);
  }
  ...
  if (!res.ok) {
    // 서버에서 JSON으로 오류를 주면 detail/message를, 아니면 raw 텍스트라도 사용
    const msg = (data && (data.detail || data.message)) || text || "요청 실패";
